...
@Metric.register("rec_precision")
class RecPrecision(Metric):
    def __init__(self, top_k: int = 1) -> None:
        if top_k <= 0:
            raise ConfigurationError("top_k passed to RecPrecision must be > 0")
        self._top_k = top_k
    ...
    def __call__(
        ...
        self.precision_sum += torch.sum(correct.float() / torch.full((batch_size,), self._top_k, dtype=torch.float32, device=gold_labels.device))
    ...
    def get_metric(self, reset: bool = False):
        ...
        if self.total_count > 1e-12:
            precision = float(self.precision_sum) / float(self.total_count)
        else:
            precision = 0.0
        if reset:
            self.reset()
