...
class HammingLoss(Metric):
    ...
    def __call__(
        ...
        xor_tensor = predictions_bin ^ gold_labels

        # This is of shape (batch_size, ..., top_k).
        xor_count = torch.sum(xor_tensor, -1)
        
        self.loss_sum += torch.sum(xor_count.float() / num_classes)
    ...
    def get_metric(self, reset: bool = False):
        ...
        if self.total_count > 1e-12:
            precision = float(self.loss_sum) / float(self.total_count)
        else:
            precision = 0.0
        if reset:
            self.reset()
